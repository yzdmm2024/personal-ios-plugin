// 巨魔(TrollStore)专用 dylib: 移除 substrate.h, 不依赖 CydiaSubstrate
...
static void postTouch(CGFloat x, CGFloat y, BOOL touchDown) {
    NSLog(@"[AC DEBUG] postTouch: x=%.1f y=%.1f down=%d", x, y, touchDown);
    // 必须在主线程调用
    ...
    IOHIDEventRef parent = $IOHIDEventCreateDigitizerEvent(kCFAllocatorDefault, mach_absolute_time(),
        kIOHIDDigitizerTransducerTypeHand, 99, 1, 0, 0,
    ...
    if ($clientDispatch) {
        $clientDispatch(g_iohidClient, parent);
    }
    CFRelease(parent);
}
...
- (void)startRecording {
    ...
    // 关闭连点器面板，避免影响录制
    [self dismissPanelFast];
    // Hook sendEvent: to capture touches (method swizzling, TrollStore-safe)
    swizzleSendEvent(YES);
    // 更新录制按钮状态
...
static void swizzleSendEvent(BOOL hook) {
    ...
    if (hook) {
        ...
        class_replaceMethod(cls, sel, (IMP)hook_sendEvent, method_getTypeEncoding(m));
    } else if (orig_sendEvent) {
        class_replaceMethod(cls, sel, (IMP)orig_sendEvent, method_getTypeEncoding(m));
    }
}
